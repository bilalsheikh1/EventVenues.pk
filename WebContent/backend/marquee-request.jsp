...
setTimeout(
	...
	function approved(){
	      ...
	      $.ajax({
	          url : '../Marquee' , 
	          data : "id="+marqueeID+"&method=approved" ,
	          type : 'post' ,
  ...
  function disApproved (){
	if(marqueeID > 0 ){ 
	   ...
	   $.ajax({
      		url : '../Marquee' ,
      		data : "id="+marqueeID+"&reason="+reason+"&method=disApproved" ,
      		type : 'post' ,
	...
	<div class="modal-dialog modal-confirm">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<p><b>Reason</b></p>
				<textarea style="width: 90%" rows="8" name="reason" id="reason"></textarea>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-info" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-danger" onclick="disApproved();" data-dismiss="modal">Reject</button>
			</div>
		</div>
	</div>
</div> 
	...
	<div class="modal-dialog modal-confirm">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Are you sure? To Accept This Marquee</h4>	
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			</div>
			<div class="modal-body">
				<p>Do you really want to delete these records? This process cannot be undone.</p>
			...
			<div class="modal-footer"> 
				<button type="button" class="btn btn-info" data-dismiss="modal">Cancel</button>
				<button type="button" class="btn btn-success" onclick="approved();" data-dismiss="modal">Accept</button>
			</div>
		</div>
	</div>
</div> 
...
<jsp:include page="include/sidebar.jsp"></jsp:include>
  ...
  <div class="box">
            <div class="box-header"> 
              <h3 class="box-title">Marquee Request</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
          ...
          String d = sdf.format(date);
                ...
                <tr>
                  <td><%=serialNo%></td>
                    ...
                    <td><%=m.getAddress() %></td>
                    <td><%=m.getContact() %></td>
                    <td><a href="#my_Modal" class="trigger-btn" data-toggle="modal"><input type="image" id="image" src="img/checked.png" onclick="getID(<%=m.getId()%>)" /></a>
                         <a href="#myModal" class="trigger-btn" data-toggle="modal"><input type="image" id="image" src="img/cancel.png" onclick="getID(<%=m.getId() %>)"  /></a>
                       <a href="marquee-detail.jsp?id=<%=m.getId()%>Mb<%=d.replaceAll("\\s", "")%>Ha3g7"><input type="image" id="image" src="img/detail.png" /></a>
                    </td>
                </tr>
                <%} %>
                </tbody>
